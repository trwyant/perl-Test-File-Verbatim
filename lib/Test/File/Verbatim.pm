...
sub __ignore_file {
    ...
    state $is = { map { $_ => 1 } '.DS_Store', '.git' };
    $is->{$_}
	and return 1;
    state $ext = { map { $_ => 1 } qw{ bak gif gz jpeg jpg mo pdf pickle pkl png pyc pyd pyo so tar tgz zip } };
    my ( $extension ) = m/ [.] ( [^.]+ ) \z /smx;
    $ext->{$extension}
	and return 1;
    return m/~$|^#.+#$|[._].*[.]swp$|core[.]\d+$|[.-]min[.]js$|[.]js[.]min$|[.]min[.]css$|[.]css[.]min$|[.]js[.]map$|[.]css[.]map$/;
}
