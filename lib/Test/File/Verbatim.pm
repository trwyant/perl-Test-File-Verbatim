...
sub all_verbatim_ok {
    ...
    defined $opt->{exclude}
	or $opt->{exclude} = -e 'MANIFEST.SKIP' ?
	    \&exclude_manifest_skip :
	    \&exclude_ack_ignore;
    my $rslt = 1;
    foreach my $path ( sort map { $self->_all_verbatim_ok_expand( $opt, $_ ) } @arg ) {
...
# Generated by tools/extract-ack-ignore from App::Ack::ConfigDefault,
# which is part of ack v3.6.0
sub exclude_ack_ignore {
    use File::Spec;
    state $ignore = {
      'dir' => {
        'is' => {
      ...
      'file' => {
        ...
        'match' => qr/[.-]min[.]js$|[.]css[.]map$|[.]css[.]min$|[.]js[.]map$|[.]js[.]min$|[.]min[.]css$|[._].*[.]swp$|^#.+#$|core[.]\d+$|~$/,
      },
    };

    my $what = -d _ ? 'dir' : 'file';
    local $_ = ( File::Spec->splitpath( $_ ) )[2];
    $ignore->{$what}{is}{$_}
	and return 1;
    if ( ( my $dot = rindex $_, '.' ) >= 0 ) {
	my $ext = substr $_, $dot + 1;
	$ignore->{$what}{ext}{$ext}
	    and return 1;
    }
    $ignore->{$what}{match}
	or return 0;
    return $_ =~ $ignore->{$what}{match};
}

sub exclude_manifest_skip {
    state $skip = do {
	my @re;
	local $! = undef;
...
This option specifies files to be excluded from the test. This test
applies to the path name of the file relative to the distribution's base
...
A file is excluded if its path matches the given regular expression.

=back

=back

Any other value will cause the test to BAIL_OUT.

If unspecified, this defaults to subroutine
L<exclude_manifest_skip()|/exclude_manifest_skip> if file
F<MANIFEST.SKIP> exists; otherwise it defaults to
L<exclude_ack_ignore()|/exclude_ack_ignore>.

If you do not want any files ignored by default, specify C<sub { 1 }>.
...
=head2 exclude_ack_ignore

This subroutine is B<not> a test. It is exclusion code for
L<all_verbatim_ok()|/all_verbatim_ok>.

This subroutine is generated from the
L<App::Ack::ConfigDefault|App::Ack::ConfigDefault> C<--ignore-file>,
C<--ignore-dir>, and C<--ignore-directory> (sic) specifications by
F<tools/extract-ack-ignore>.

This subroutine is exposed so that you can
...
=head2 exclude_manifest_skip

This subroutine is B<not> a test. It is exclusion code for
L<all_verbatim_ok()|/all_verbatim_ok>.

This subroutine excludes files based on the contents of the
F<MANIFEST.SKIP> file. If this file does not exist, this subroutine
accepts all files.

This subroutine is exposed so that you can specify it explicitly.
...
Copyright (C) 2022, 2024-2026 by Thomas R. Wyant, III
